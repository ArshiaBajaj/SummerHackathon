import type { CommentaryStyle } from "@/state/gameStore";
...
const PLAYGROUND = {
  score2: [
    ...
    "Team {T} splashes it. Score is {A} to {B}.",
  ],
  score3: [
    "From downtown! Team {T} drills the three, {A}–{B}.",
    "Long ball, all net! Team {T} up to {S}.",
  ...
  streak: [
    ...
    "Team {T} is cooking. {N} unanswered.",
  ],
  outOfBounds: [
...
const BROADCAST = {
  score2: [
    ...
    "Confirmed basket, Team {T}. Now {A}–{B}.",
  ],
  score3: [
    "Three-point field goal, Team {T}. {A}–{B}.",
    "Perimeter shot connects for Team {T}.",
    "That's a three. Team {T} extends to {S}.",
  ],
  streak: ["Team {T} scores {N} consecutive possessions.", "Momentum shift — Team {T} on a {N}-point run."],
  outOfBounds: [
...
const HYPE = {
  score2: [
    ...
    "LET'S GOOO! Team {T}, {A} to {B}!",
  ],
  score3: [
    "BANG! Team {T} drops the three!",
    "SPLASH CITY! Team {T} up to {S}!",
    "OUTRAGEOUS! Deep three, Team {T}!",
  ],
  streak: ["THIS IS A TAKEOVER! {N} STRAIGHT FOR TEAM {T}!", "TEAM {T} IS UNCONSCIOUS! {N} IN A ROW!"],
  outOfBounds: ["OFF! Ball goes the other way, Team {T}!", "OUT! Fresh possession!"],
  jump: ["{N}CM! HE FLEW!", "OFF THE PLANET — {N}CM VERTICAL!"],
  release: ["{V} M/S OFF THE WRIST!", "SNAP RELEASE — {V} METERS PER SECOND!"],
  intro: ["We are LIVE from the park. Let's ride!", "Anact Ortho engaged — showtime!"],
};
...
function tableFor(style: CommentaryStyle): typeof PLAYGROUND {
  if (style === "broadcast") return BROADCAST as typeof PLAYGROUND;
  if (style === "hype") return HYPE as typeof PLAYGROUND;
  return PLAYGROUND;
}

export function scoreLine(style: CommentaryStyle, c: Ctx): string {
  const t = tableFor(style);
  const arr = c.points === 3 ? t.score3 : t.score2;
  return fill(pick(arr), c);
}

export function streakLine(style: CommentaryStyle, c: Ctx & { streak: number }): string {
  return fill(pick(tableFor(style).streak), c);
}

export function outLine(style: CommentaryStyle, c: Ctx): string {
  return fill(pick(tableFor(style).outOfBounds), c);
}

export function jumpLine(style: CommentaryStyle, c: Ctx & { jumpCm: number }): string {
  return fill(pick(tableFor(style).jump), c);
}

export function releaseLine(style: CommentaryStyle, c: Ctx & { releaseMps: number }): string {
  return fill(pick(tableFor(style).release), c);
}

export function introLine(style: CommentaryStyle): string {
  return pick(tableFor(style).intro);
}
